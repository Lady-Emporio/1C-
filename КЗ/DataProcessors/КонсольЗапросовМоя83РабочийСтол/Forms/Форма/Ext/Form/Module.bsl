...
Процедура ПреобразоватьИзКода(Команда)
	ИсходныйТекст = рфТекстЗапроса.ПолучитьТекст();
	ИсходныйТекст = СтрЗаменить(ИсходныйТекст, """""", """");
	СписокСтрок = СтрРазделить(ИсходныйТекст, Символы.ПС);
	Для счетчик = 0 по СписокСтрок.Количество() -1 цикл
		ОбрабатываемаяСтр = СписокСтрок[Счетчик];
		ОбрабатываемаяСтр = СокрЛП(ОбрабатываемаяСтр);
		Если СтрНачинаетсяС(ОбрабатываемаяСтр, "|") тогда
			ОбрабатываемаяСтр = Сред(ОбрабатываемаяСтр, 2);	
		КонецЕсли;
		СписокСтрок[Счетчик] = ОбрабатываемаяСтр;
	КонецЦикла;
	ИсходныйТекст = СтрСоединить(СписокСтрок, Символы.ПС);
...
&НаКлиенте
Процедура ФормаОткрытьФормаHTTPЗапросов(Команда)
	ОткрытьФорму(Объект.ПутьКФормам+".ФормаHTTPЗапросов", ,, Новый УникальныйИдентификатор(),,,,РежимОткрытияОкнаФормы.Независимый);
КонецПроцедуры

&НаКлиенте
Процедура КонтекстноеМеню_Тип(Команда)
	
	Тип = Сред(Элементы.рфТабДокКонтекстноеМенюКонтекстноеМеню_Тип.Заголовок, СтрДлина("Тип: ")+1);
	Примечание = Элементы.рфТабДок.ТекущаяОбласть.Примечание.Текст;
	Если НЕ ПустаяСтрока(Примечание) тогда
		Тип = Элементы.рфТабДок.ТекущаяОбласть.Примечание.Текст;		
	КонецЕсли;
	Сообщить(Тип);
...
&НаКлиенте
Процедура ФормаОткрытьФормаАнализМетаданных(Команда)
	
	ОткрытьФорму(Объект.ПутьКФормам+".ФормаАнализМетаданных", ,, Новый УникальныйИдентификатор(),,,,РежимОткрытияОкнаФормы.Независимый); 
...
Процедура рфТабДокВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ТипЗнч(Область) = Тип("РисунокТабличногоДокумента") тогда
		...
		СтруктураТекст = Новый Структура("Текст", Неопределено);
		ЗаполнитьЗначенияСвойств(СтруктураТекст, Область);
		Если ЗначениеЗаполнено(СтруктураТекст.Текст) тогда
			Сообщить(СтруктураТекст.Текст);
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Процедура рфПараметрыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	
	Отказ=Истина; 
	
	Стр = рфПараметры.Добавить();
	Стр.Тип = Новый ОписаниеТипов();//Если указать ОписаниеТипов как тип реквизита, то при сериализации все возможные типы будут указаны. Слишком много места займет.
	Если Копирование тогда
		ПрошлыеПараметры = Элементы.рфПараметры.ТекущиеДанные;
		ЗаполнитьЗначенияСвойств(Стр, ПрошлыеПараметры);		
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура рфТабДокОбработкаДополнительнойРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	Элементы.рфТабДокКонтекстноеМенюКонтекстноеМеню_Тип.Заголовок = "Тип: " + Расшифровка; 
КонецПроцедуры
...
Функция ВыполнитьНаСервереОтдельныйЗапрос(ТекстЗапроса, ТЗ_Параметры, ТипВыполнения, МенеджерВременныхТаблиц)
	...
	Для СчетчикЗапросов = 0 по СписокПакетов.Количество()-1 цикл 
		...
		Для счетчик=0 по КолКолонок Цикл
			...
			#Область ТипВИмяКолонки //Вывод тип в расшифровку имени колонки
			ТипыКолонки = Новый Массив;
			ВсеТипыПеречисления = Перечисления.ТипВсеСсылки();
			Для каждого ТипКолонки из Колонки[счетчик].ТипЗначения.Типы() цикл
				Если ВсеТипыПеречисления.СодержитТип(ТипКолонки) тогда
					МетаПеречисление = Метаданные.НайтиПоТипу(ТипКолонки);
					ТипыКолонки.Добавить(МетаПеречисление.ПолноеИмя() + ":");
					Для каждого ЗначПеречисления из МетаПеречисление.ЗначенияПеречисления цикл
						ТипыКолонки.Добавить(СтрШаблон("____%1(%2)",ЗначПеречисления.Имя, ЗначПеречисления.Синоним));		
					КонецЦикла;
				Иначе 
					МетаОб = Метаданные.НайтиПоТипу(ТипКолонки);	
					Если МетаОб = Неопределено тогда
						СтрТип = Строка(ТипКолонки);
					Иначе
						СтрТип = МетаОб.ПолноеИмя();	
					КонецЕсли;
					ТипыКолонки.Добавить(СтрТип);	
				КонецЕсли;
			КонецЦикла; 
			Ячейка.Области.Ячейка.Примечание.Текст = СтрСоединить(ТипыКолонки, Символы.ПС); 
			#КонецОбласти //ТипВИмяКолонки
			...
			КонецЕсли;
			Ячейка.Области.Ячейка.Примечание.Текст = "";
		КонецЦикла; 
		...
		Пока Выборка.Следующий() Цикл
			Для счетчик=0 по КолКолонок Цикл
				...
				ЗнчПоля = Выборка[Колонки[счетчик].Имя];
				Если ТипЗнч(ЗнчПоля) = Тип("ХранилищеЗначения") тогда
					ВозможноХранилище = Выборка[Колонки[счетчик].Имя];
					СодержимоеХранилища = ВозможноХранилище.Получить();
					Запись = Новый ЗаписьXML;
					Запись.УстановитьСтроку();
					СериализаторXDTO.ЗаписатьXML(Запись, СодержимоеХранилища);
					Ячейка.Параметры.Ячейка = Запись.Закрыть();
					
				ИначеЕсли ТипЗнч(ЗнчПоля) = Тип("Null") тогда
					Ячейка.Параметры.Ячейка = "<NULL>";
					ОформлениеЯчейки_НеопределеноNull(Ячейка);
				ИначеЕсли ТипЗнч(ЗнчПоля) = Тип("Неопределено") тогда
					Ячейка.Параметры.Ячейка = "<Неопределено>";
					ОформлениеЯчейки_НеопределеноNull(Ячейка);
				Иначе
					Ячейка.Параметры.Ячейка = ЗнчПоля;	
				КонецЕсли;
				
				ОформлениеЯчейки_ВыводТипа(Ячейка, ЗнчПоля);	
				...
				Иначе
					ТабДок.Присоединить(Ячейка);
				КонецЕсли;
				ОформлениеЯчейки_Сбросить(Ячейка);
	...
	Если ИдСтроки <> Неопределено тогда 
		...
		Если ПустаяСтрока(ТекДанные.Имя) тогда
			...
			Если ТипЗнч(ОписаниеПоследней) <> Тип("ЗапросУничтоженияТаблицыСхемыЗапроса") тогда
...
#Область ОкрашиваниеТабДок

Процедура ОформлениеЯчейки_Сбросить(Ячейка)
	Ячейка.Области.Ячейка.ЦветТекста = Новый Цвет();
	Ячейка.Области.Ячейка.Примечание.Текст = "";
КонецПроцедуры

Процедура ОформлениеЯчейки_НеопределеноNull(Ячейка)
	Ячейка.Области.Ячейка.ЦветТекста = WebЦвета.ТусклоСерый;
КонецПроцедуры

Процедура ОформлениеЯчейки_ВыводТипа(Ячейка, Значение)
	
	ТекТип = ТипЗнч(Значение);
	МетаОб = Метаданные.НайтиПоТипу(ТекТип);	
	Если МетаОб = Неопределено тогда
		Результат = Строка(ТекТип);
	Иначе
		Результат = МетаОб.ПолноеИмя();
		Если СтрНачинаетсяС(Результат, "Перечисление.") И НЕ Значение.Пустая() тогда
			ИндексЗначенияПеречисления = Перечисления[МетаОб.Имя].Индекс(Значение);
			ИмяЗначенияПеречисления = Метаданные.Перечисления[МетаОб.Имя].ЗначенияПеречисления[ИндексЗначенияПеречисления].Имя;
			Результат = МетаОб.ПолноеИмя() + "." + ИмяЗначенияПеречисления;
		КонецЕсли;
	КонецЕсли;
	
	Если рфВывестиТипы тогда
		Ячейка.Области.Ячейка.Примечание.Текст = Результат;
	КонецЕсли;
	Ячейка.Параметры.МояРасшифровка = Результат;
	
КонецПроцедуры

#КонецОбласти //ОкрашиваниеТабДок
