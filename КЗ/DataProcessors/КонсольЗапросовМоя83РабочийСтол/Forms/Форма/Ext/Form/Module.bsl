...
Процедура СписокЗапросовПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Копирование тогда
		...
		ИндексПрошлого = рфСписокЗапросов.Индекс(ПрошлыйЗапрос)+1;
		Стр=рфСписокЗапросов.Вставить(ИндексПрошлого);
	Иначе
		Стр=рфСписокЗапросов.Добавить();	
	КонецЕсли;
...
Функция СохранитьДанныеВФайл_Сервер(СохраненныйТекст)
	...
	Для каждого стрЗапрос из рфСписокЗапросовОб цикл
		Для каждого стрПар из стрЗапрос.Параметры цикл
			...
			КонецЦикла;      
			Если лРеквизитНайден тогда
				СохраняемыеДанныеПоБазе.ТЗ_Запросов.Вставить(СтрПар.Значение, РеквизитФормыВЗначение(СтрПар.Значение));
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	...
	Соответствие.Вставить(СтрокаСоединенияИнформационнойБазы(), СохраняемыеДанныеПоБазе_XML);
...
Процедура ПриОткрытии_ЗагрузкаСохраненногоЗапроса(СохраненныйТекст)
	...
	ДанныеПоТекБазе_Строка = Соответствие.Получить(СтрокаСоединенияИнформационнойБазы());
	Если ДанныеПоТекБазе_Строка <> неопределено Тогда 
		Чтение = Новый ЧтениеXML;
