...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.рфТипОбъекта.СписокВыбора.Очистить();
	...
	Для каждого Менеджер из СписокМенеджеров цикл
		Для каждого МетаМенеджер из Менеджер цикл
			Элементы.рфТипОбъекта.СписокВыбора.Добавить(МетаМенеджер.ПолноеИмя(), МетаМенеджер.ПолноеИмя() + " | " + МетаМенеджер.Представление());		
		КонецЦикла;   
	КонецЦикла;
...
Процедура рфТипОбъектаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("Строка") тогда
		ОбработкаВведенногоТипа(ВыбранноеЗначение)
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура рфТипОбъектаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	
	Если Ожидание > 0 тогда
		СтандартнаяОбработка = ложь;
		ДанныеВыбора = Новый СписокЗначений;    
		лТекст = нрег(Текст);
		Для каждого стр из Элементы.рфТипОбъекта.СписокВыбора цикл
...
&НаКлиенте
Процедура рфТипОбъектаПриИзменении(Элемент)
	
	ОбработкаВведенногоТипа(рфТипОбъекта);
...
Процедура ОбработкаВведенногоТипа(ТипСтрокой)
	СуществованиеМета = Метаданные.НайтиПоПолномуИмени(ТипСтрокой);
	Если СуществованиеМета = Неопределено тогда
		Сообщить(СтрШаблон("Не существует типа: ""%1"".",ТипСтрокой));
		Возврат;
	КонецЕсли;
	рфТипОбъекта = ТипСтрокой;
	ЗапускГенерацияДинСписка("рфДинСписок");
